...
let sectionGetter = document.querySelector("#pictures")

for (let item of picArray) {
  let createArticle = document.createElement("Article");
  createArticle.classList.add("card");
  sectionGetter.appendChild(createArticle);

  let createH2 = document.createElement("h2");
  createH2.innerText = item.title;
  ...
  createImg.src = item.image.medium;
  createImg.alt = item.image.title
  createFigure.appendChild(createImg);

  let createFigcaption = document.createElement("figcaption");
  createFigcaption.innerText = item.caption
  createFigure.appendChild(createFigcaption);

  let createPara = document.createElement("p");
  createPara.innerText = item.description;
